...
class NewsAdapter : RecyclerView.Adapter<NewsAdapter.MyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        val dateResultFormat = SimpleDateFormat("EEE, dd MMM", Locale.getDefault()) // DAY, tgl Year
            .format(calendar.time).toString()
        // section pager adapter
        val timeResultFormat = SimpleDateFormat("HH:mm", Locale.getDefault())
            .format(calendar.time).toString()

        // for print log info on the logcat, especially for date
...
//                .into(ivNews)
            Picasso
                ...
                .resize(2048, 1600)
                .onlyScaleDown()
                .placeholder(R.drawable.logo_moslemnews)
                .into(ivNews)
        }
